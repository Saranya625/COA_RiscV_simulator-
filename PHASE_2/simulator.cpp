...
void fetchInstruction(vector<Core>& cores, int &pc) {  // Pass pc as reference
    ...
    for (Core &core : cores) {       
        ...
        if (core.global_pc != 0) {
            ...
            if (core.core_id == core.cid_val) {
                if(pc == core.global_pc){
                    cout<< "[Core "<< core.core_id<<"continues execution with other cores.."<<endl;
                   ...
                   core.global_pc=0;
                    ...
                    std::cout << "[Core " << core.core_id << "] Skipping execution of " << instructions[pc].first << "\n";
                    cout << "[Core " << core.core_id << "] Skipping execution of " << core.pipeline.ID_EX.instruction << "\n";    
                    core.pipeline.IF_ID.valid_instruction=false;
                    core.pipeline.ID_EX.valid_instruction = false;
       ...
       // Fetch instruction for this core
        instr = instructions[pc].first;
        args =instructions[pc].second;
        core.pipeline.IF_ID.instruction = instructions[pc].first;
        core.pipeline.IF_ID.args = instructions[pc].second;
        core.pipeline.IF_ID.valid_instruction = true;
        core.pipeline.IF_ID.valid_data = false;
    }
    pc++;   
   
    std::cout << "Fetched instruction: " << instr << " at PC: " << pc << "\n";
    
    cout << "PC after fetch: " << pc << endl;
}
...
void executePipeline(vector<Core>& cores) {
    ...
    while (running) {
        ...
        for (Core &core : cores) {
         ...
         fetchInstruction(cores,global_pc);
        for (Core &core : cores) {
