...
void parseAssembly(const std::string &filename,int core_id) {
    ...
    int memoryAddress= heap_pointers[core_id] ;
    ...
    while (std::getline(file, line)) {
        ...
        if (inDataSection) {
            if (first.back() == ':') {
                ...
                std::cout << "Stored label: " << first << " at address " << memoryAddress << "for core:" << core_id << std::endl;
            ...
            if (instr == ".word") {
                int size = args.size();
                memoryAddress = allocate_memory(size * 4 , core_id); // Allocate memory for .word
                for (size_t j = 0; j < args.size(); j++) {
                    int value = std::stoi(args[j]);
                    sw(memoryAddress+(j * 4), value, core_id);
                }
            }
        } else if (inTextSection) {
            if (first.back() == ':') {
                first.pop_back();
                label_map[first] = instructionIndex;
