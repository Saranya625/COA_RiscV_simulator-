        ...
        if(stall_flag== true){
            ...
            if(EX_MEM.remaining_cycles >1){
              ...
              cout<<" EX_MEM.extra_cycles:"<< EX_MEM.extra_cycles<<endl;
               ...
               /*
                if(EX_MEM.extra_cycles !=0 ){
                    cout<<"Executing last cycle"<<endl;               
                    WB_Return= MEM_WB;
                    MEM_WB.valid_instruction = true;
                    //EX_MEM=EX_MEM;
                    ID_EX = IF_ID;
                    IF_ID = PipelineStage();
                    IF_ID.valid_instruction = false;
                    IF_ID.valid_data = false;
                    EX_MEM.extra_cycles=0;
                    cout<<" EX_MEM.extra_cycles:"<< EX_MEM.extra_cycles<<endl;
                    return;
                }
               */
...
public:
    ...
    Core(int id, std::unordered_map<std::string, int> &labels, bool forwarding , unordered_map<string,int > latencies) : core_id(id), label_map(labels), enable_forwarding(forwarding) ,instruction_latencies(latencies){
        registers[0]=0;
         ...
         //5. IF FORWARDING IS DISABLED THEN CHECK FOR DEPENDENCY IN EX
          ...
          //5. IF FORWARDING IS DISABLED THEN CHECK FOR DEPENDENCY IN MEM
                ...
                if (label_map.find(args[2]) != label_map.end()) {
                    pipeline.ID_EX.rd = label_map[args[2]];
                } else {
                ...
                if (label_map.find(args[1]) != label_map.end()) {
                    pipeline.ID_EX.offset = label_map[args[1]];  // Target label address
                } else {
    ...
    void executeStage() {
        ...
        if (pipeline.EX_MEM.remaining_cycles > 1) {
            ...
            else if (enable_forwarding && (pipeline.WB_Return.instruction == "lw")){
            ...
            pipeline.EX_MEM.rd_value = imm >> rs1_val;
                ...
                if (rs1_val == rs2_val) {
                    ...
                    cout<<"Flushing out:"<< pipeline.ID_EX.instruction<<"due to the branch"<<endl;
                    pc = label_map[args[2]];
                ...
                pc = label_map[args[0]];
                cout << "jump to label " << args[0] << endl;
                cout<<"Flushing out:"<< pipeline.ID_EX.instruction<<"due to the branch"<<endl;
                pipeline.ID_EX.valid_instruction = false;
                pipeline.stalls++;
            } else if (instr == "jal") {
    ...
    void memoryStage() {
        ...
        std::cout << "[Core " << core_id << "] Memory stage for instruction: " << pipeline.MEM_WB.instruction << "\n";
            ...
            if (instr == "lw") {
                ...
                pipeline.MEM_WB.rd_value = lw(pipeline.MEM_WB.address+base_adress);
            } else if (instr == "sw") {
                sw(pipeline.MEM_WB.address+base_adress, pipeline.MEM_WB.rs1_value);
            }
