    ...
    while (std::getline(file, line)) {
        ...
        if (inDataSection) {
            ...
            if (instr == ".word") {
                ...
                if (!args.empty()) {
                    int allocated_address = allocate_memory(args.size());  
                    label_map[first] = allocated_address;
                    std::cout << "Allocated memory for " << first << " at 0x" 
                              << std::hex << allocated_address << std::endl;
            
                    for (size_t i = 0; i < args.size(); i++) {
                        int value = std::stoi(args[i]);
                        sw(allocated_address + (i * 4), value); // Store values in memory
                    }
                } else {
