    ...
    void executeStage() {
        ...
        if(pipeline.EX_MEM.high_latency == true){
            ...
            pipeline.EX_MEM.rd_value = imm >> rs1_val;
                ...
                if(forwarding==1){
                    pipeline.EX_MEM.rs1_value= registers[pipeline.EX_MEM.rs1];
                    forwarding = 0;
                }
                if(forwarding==2){
                    pipeline.EX_MEM.rd_value= registers[pipeline.EX_MEM.rd];
                    cout<<"changed value of rd:"<< pipeline.EX_MEM.rd_value<<endl;
                    pipeline.EX_MEM.address= pipeline.EX_MEM.rd_value + pipeline.EX_MEM.offset;
                    cout<<"new address:"<< pipeline.EX_MEM.address<<endl;
                    cout<<"new value:"<< pipeline.EX_MEM.rd_value<<endl;
                    forwarding = 0;
                }
            }
            else if( instr == "and" ){
    ...
    void writeBack() {
        ...
        if(pipeline.WB_Return.stalled){
            ...
            if (instr == "add" || instr == "sub" || instr == "addi" || instr == "slli" || instr == "lw" || instr == "li" || instr == "la"|| instr=="mul"||instr == "and"||instr=="srli"||instr == "and"||instr=="or"||instr=="xor") {
                ...
                << " = " << registers[pipeline.WB_Return.rd] << "\n";   
            }
