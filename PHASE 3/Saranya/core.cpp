...
public:
    ...
    void shiftStages() {
        if(stall_flag== true){
            ...
            if(MEM_WB.memory_remaining_cycles>1 && MEM_WB.valid_instruction){
                MEM_WB.memory_remaining_cycles--;
                ...
                MEM_WB.extra_cycles++;
                stall_flag=false;
                cout<<" MEM_WB.extra_cycles:"<< MEM_WB.extra_cycles<<endl;
...
public:
  int global_pc=0;
    ...
    Core(int id, std::unordered_map<std::string,vector< int>> &labels, bool forwarding , unordered_map<string,int > latencies , const SP_memory& spm_config , int main_mem_lat) : core_id(id), label_map(labels), 
    ...
    if(instr== "lw"|| instr == "sw"){
        ...
        pipeline.ID_EX.memory_remaining_cycles = main_memory_latency;
    ...
    void memoryStage() {
        ...
        if(pipeline.MEM_WB.high_memory_latency == true){
            if (pipeline.MEM_WB.memory_remaining_cycles == pipeline.MEM_WB.memory_latency) {
                std::cout << "[Core " << core_id << "] Memory stage for instruction: " << pipeline.MEM_WB.instruction 
                          << " (cycle:" << pipeline.MEM_WB.remaining_cycles << ") \n";
            }
            else{
                std::cout << "[Core " << core_id << "] Memory stage for instruction: " << pipeline.MEM_WB.instruction 
                << " (cycle:" << pipeline.MEM_WB.remaining_cycles << ") \n";
