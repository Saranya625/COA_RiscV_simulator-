...
void fetchInstruction(vector<Core>& cores) {  
    ...
    for (Core &core : cores) {       
        ...
        instr = fetch_instruction(core.pc);
        args = fetch_args(core.pc);
        core.pipeline.IF_ID.instruction = fetch_instruction(core.pc) ;
        core.pipeline.IF_ID.args = fetch_args(core.pc) ;
...
int main() {
    ...
    initializeSystem(specs.L1_cache_size, specs.L2_cache_size,  specs.L1_associativity, specs.L2_associativity, specs.L1_latency, specs.L2_latency, specs.replacement_policy);
